<template>
  <MainLayout>
    ...
    <div class="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div v-if="recipe.image" class="reveal mt-8 overflow-hidden rounded-2xl border border-gray-100 shadow-md">
        <div class="aspect-video">
          <img :src="recipe.image" :alt="recipe.title" class="h-full w-full object-cover" />
        </div>
      </div>

      <div class="reveal mt-8 flex flex-wrap gap-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:gap-6 sm:p-5">
        <div v-if="recipe.cooking_time" class="flex items-center gap-2 text-sm text-gray-700">
          <span class="flex h-9 w-9 items-center justify-center rounded-lg bg-[#003274]/10 text-[#003274]">
      ...
      <div v-if="recipe.city?.slug" class="reveal mt-12 border-t border-gray-200 pt-8">
