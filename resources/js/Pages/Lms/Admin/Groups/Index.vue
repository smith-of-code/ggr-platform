  ...
  <LmsAdminLayout :event="event">
    <div class="mb-8 flex items-center justify-between">
      <div>
        ...
        <p class="mt-1 text-sm text-gray-500">Группы события «{{ event.title }}»</p>
      </div>
      <Link :href="route('lms.admin.groups.create', event.slug)" class="flex items-center gap-2 rounded-xl bg-rosatom-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-rosatom-700">
    ...
    <RCard flush>
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="g in groups.data" :key="g.id" class="transition hover:bg-gray-50">
            <td class="px-5 py-3.5 text-sm font-medium text-gray-900">
              {{ g.title }}
              <RBadge v-if="g.is_city_group" variant="primary" size="sm" class="ml-2">Городская</RBadge>
            </td>
            <td class="px-5 py-3.5 text-sm text-gray-500">{{ g.curator?.name ?? '—' }}</td>
            <td class="px-5 py-3.5 text-sm text-gray-500">
              <span v-if="g.is_city_group && g.city?.name">{{ g.city.name }}</span>
              <span v-else-if="(g.linked_cities || []).length" class="line-clamp-2">{{ (g.linked_cities || []).join(', ') }}</span>
              <span v-else>—</span>
            </td>
            <td class="px-5 py-3.5 text-center text-sm text-gray-500">{{ g.members_count ?? 0 }}</td>
            <td class="px-5 py-3.5 text-right">
              <div class="flex items-center justify-end gap-2">
                <Link :href="route('lms.admin.groups.edit', [event.slug, g.id])" class="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 hover:text-gray-900">
                ...
                <RButton v-if="!g.is_city_group" variant="danger" size="sm" iconOnly @click="confirmDestroy(g)">
                  <template #icon>
      ...
      <div v-if="groups.data.length === 0" class="px-5 py-16 text-center text-sm text-gray-500">Групп пока нет</div>
    </RCard>
  </LmsAdminLayout>
</template>
...
const props = defineProps({ event: Object, groups: Object })

function confirmDestroy(group) {
  if (confirm(`Удалить группу "${group.title}"?`)) {
    router.delete(route('lms.admin.groups.destroy', [props.event.slug, group.id]))
  }
}
</script>
