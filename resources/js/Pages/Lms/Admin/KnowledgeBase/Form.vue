        ...
        <div class="space-y-3 p-8">
          <div v-for="(item, idx) in form.items" :key="idx" class="rounded-xl border border-gray-200 bg-gray-50 p-4">
            <div class="flex justify-between gap-2">
              <RInput v-model="item.title" placeholder="Название" required size="sm" class="min-w-0 flex-1" />
              <div class="flex shrink-0 items-start gap-1 pt-0.5">
                ...
                <RButton v-if="idx > 0" type="button" variant="ghost" size="sm" iconOnly title="Выше" @click="moveKbItem(idx, -1)">
                  <template #icon><ChevronUpIcon class="h-4 w-4" /></template>
                </RButton>
                <RButton v-if="idx < form.items.length - 1" type="button" variant="ghost" size="sm" iconOnly title="Ниже" @click="moveKbItem(idx, 1)">
                  <template #icon><ChevronDownIcon class="h-4 w-4" /></template>
                </RButton>
                <button type="button" class="mt-0.5 cursor-pointer text-gray-400 hover:text-red-500" title="Удалить" @click="form.items.splice(idx, 1)">
