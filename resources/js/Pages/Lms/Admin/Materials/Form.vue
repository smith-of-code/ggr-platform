          ...
          <div v-if="form.files.length" class="space-y-3">
            ...
            >
              <div class="flex-1 space-y-2">
                ...
                <div v-else-if="file._uploading" class="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
                ...
                <label v-else class="group flex cursor-pointer items-center gap-2 rounded-lg border-2 border-dashed border-gray-300 bg-white px-4 py-3 transition hover:border-rosatom-400 hover:bg-rosatom-50/30">
                  ...
                  <input type="file" class="hidden" @change="e => uploadMaterialFile(e, idx)" />
                </label>
              </div>
...
async function uploadMaterialFile(e, idx) {
  ...
  try {
    ...
    entry.file_path = data.url
    entry.file_name = data.name
    entry.file_size = file.size
    if (!entry.title) entry.title = data.name
  } catch (err) {
