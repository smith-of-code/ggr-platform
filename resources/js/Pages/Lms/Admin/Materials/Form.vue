          ...
          <div v-if="form.files.length" class="space-y-3">
            ...
            >
              ...
              <div class="flex shrink-0 items-start gap-1 pt-0.5">
                <RButton v-if="idx > 0" type="button" variant="ghost" size="sm" iconOnly title="Выше" @click="moveMaterialFile(idx, -1)">
                  <template #icon><ChevronUpIcon class="h-4 w-4" /></template>
                </RButton>
                <RButton v-if="idx < form.files.length - 1" type="button" variant="ghost" size="sm" iconOnly title="Ниже" @click="moveMaterialFile(idx, 1)">
                  <template #icon><ChevronDownIcon class="h-4 w-4" /></template>
                </RButton>
                <button type="button" class="mt-0.5 cursor-pointer text-gray-400 hover:text-red-500" title="Удалить" @click="form.files.splice(idx, 1)">
...
function moveMaterialFile(idx, delta) {
  const newIdx = idx + delta
  if (newIdx < 0 || newIdx >= form.files.length) return
  const arr = [...form.files]
