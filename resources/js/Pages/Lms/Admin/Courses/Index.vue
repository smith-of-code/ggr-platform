        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="course in courses.data" :key="course.id" class="transition hover:bg-gray-50">
            ...
            <td class="px-5 py-3.5 text-center">
              <RBadge :variant="course.is_active ? 'primary' : 'neutral'" :dot="true">
                {{ course.is_active ? 'Активен' : 'Скрыт' }}
              </RBadge>
            </td>
            <td class="px-5 py-3.5 text-right">
              <div class="flex items-center justify-end gap-2">
                <Link :href="route('lms.admin.courses.edit', [event.slug, course.id])" class="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 hover:text-gray-900">
                ...
                <RButton variant="danger" size="sm" icon-only @click="confirmDestroy(course)">
                  <template #icon>
