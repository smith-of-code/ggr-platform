<template>
  <LmsAdminLayout :event="event">
    <div class="mb-8 flex items-center justify-between">
      ...
      <Link
        v-if="canManageTests"
        :href="route('lms.admin.tests.create', event.slug)"
          ...
          <tr v-for="test in tests.data" :key="test.id" class="transition hover:bg-gray-50">
            ...
            <td class="px-5 py-3.5 text-right">
              <div class="flex items-center justify-end gap-2">
                ...
                <Link
                  v-if="canManageTests"
                  :href="route('lms.admin.tests.edit', [event.slug, test.id])"
                  class="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 hover:text-gray-900"
                >
                ...
                <RButton v-if="canManageTests" variant="danger" size="sm" icon-only @click="confirmDestroy(test)">
                  <template #icon>
...
const props = defineProps({
  event: Object,
  tests: Object,
  canManageTests: { type: Boolean, default: true },
})
