<template>
  <AdminLayout>
    ...
    <RCard elevation="raised" flush>
          ...
          <tr v-for="city in cities.data" :key="city.id" class="transition hover:bg-gray-50/50">
            ...
            <td class="px-5 py-3.5 text-right">
              <div class="flex items-center justify-end gap-2">
                ...
                <RButton variant="danger" size="sm" icon-only @click="confirmDestroy(city)">
                  <template #icon>
      ...
      <div v-if="cities.last_page > 1" class="flex items-center justify-between border-t border-gray-100 px-5 py-3">
        <p class="text-xs text-gray-500">
          {{ cities.from }}–{{ cities.to }} из {{ cities.total }}
        </p>
