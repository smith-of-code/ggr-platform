<template>
  <AdminLayout>
    ...
    <RCard elevation="raised" flush>
      ...
      <div v-if="vacancies.last_page > 1" class="flex items-center justify-between border-t border-gray-100 px-5 py-3">
        <p class="text-xs text-gray-500">
          {{ vacancies.from }}–{{ vacancies.to }} из {{ vacancies.total }}
        </p>
        <div class="flex gap-1">
          <button
            v-for="link in vacancies.links"
            :key="link.label"
            type="button"
            :disabled="!link.url"
