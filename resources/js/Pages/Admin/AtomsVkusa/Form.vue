      ...
      <div v-show="activeTab === 'results'" class="space-y-6">
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6">
          <div class="mb-4 flex items-center justify-between">
            ...
            <button type="button" class="rounded-lg bg-[#003274] px-3 py-1.5 text-xs font-medium text-white hover:bg-[#004090]" @click="addItem('results_videos', {url:'', title:'', thumbnail:''})">+ Добавить</button>
          </div>
          <div v-for="(item, i) in form.results_videos" :key="i" class="mb-3 flex items-start gap-3">
            <div class="flex-1 space-y-2">
              ...
              <ImageUploadCrop v-model="item.thumbnail" label="Обложка (постер)" :upload-url="route('admin.upload.image')" :media-picker-url="route('admin.media.index')" collection="atoms_vkusa" preview-class="h-32 w-full object-cover" :skip-crop="true" />
            </div>
