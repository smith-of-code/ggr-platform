<template>
  <AdminLayout>
    ...
    <div class="mb-8 flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Рецепты</h1>
        <p class="mt-1 text-sm text-gray-500">Рецепты атомных городов</p>
      </div>
      <Link
    ...
    <RCard elevation="raised" flush>
      <table class="min-w-full">
        <thead>
          <tr class="border-b border-gray-100 bg-gray-50/50">
            ...
            <th class="px-5 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-400">Сложность</th>
        ...
        <tbody class="divide-y divide-gray-50">
          <tr v-for="recipe in recipes.data" :key="recipe.id" class="transition hover:bg-gray-50/50">
            <td class="px-5 py-3.5">
              <div v-if="recipe.image" class="h-10 w-14 shrink-0 overflow-hidden rounded-lg bg-gray-100">
            ...
            <td class="px-5 py-3.5">
              ...
              <p class="mt-0.5 font-mono text-xs text-gray-400">{{ recipe.slug }}</p>
            </td>
            <td class="px-5 py-3.5 text-sm text-gray-600">
              {{ recipe.city?.name ?? '—' }}
            </td>
            <td class="px-5 py-3.5 text-center">
              <RBadge :variant="difficultyVariant(recipe.difficulty)" size="sm">
                {{ difficultyLabel(recipe.difficulty) }}
              </RBadge>
            </td>
            <td class="px-5 py-3.5 text-center">
              <RBadge v-if="recipe.is_published" variant="success" size="sm">Опубликовано</RBadge>
            ...
            <td class="px-5 py-3.5 text-right">
              <div class="flex items-center justify-end gap-1">
                ...
                <RButton variant="danger" size="sm" icon-only title="Удалить" @click="confirmDestroy(recipe)">
                  <template #icon>
                    <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                      <path
                        stroke-linecap="round"
      ...
      <div v-if="recipes.data.length === 0" class="px-5 py-16 text-center text-sm text-gray-400">Рецептов пока нет</div>

      <div v-if="recipes.last_page > 1" class="flex items-center justify-between border-t border-gray-100 px-5 py-3">
        <p class="text-xs text-gray-500">
          {{ recipes.from }}–{{ recipes.to }} из {{ recipes.total }}
        </p>
...
function difficultyVariant(key) {
  if (key === 'easy') return 'success'
  if (key === 'hard') return 'warning'
  return 'neutral'
}

function confirmDestroy(recipe) {
