<template>
  <div class="min-h-dvh bg-gradient-to-b from-slate-100 to-slate-50 font-sans text-slate-900">
    <Head title="Архив конкурсы — ЛК туров" />
    <TourCabinetHeader max-width-class="max-w-4xl">
      <template #title>
        <h1 class="text-xl font-bold leading-tight tracking-tight text-slate-900 sm:text-2xl lg:text-3xl">Архив конкурсы</h1>
      </template>
      <template #subtitle>
        <p class="text-sm leading-relaxed text-slate-600">Отправленные заявки на конкурс. Только для чтения.</p>
      </template>
      <template #toolbar>
    ...
    <div class="mx-auto max-w-4xl px-3 pb-10 pt-4 sm:px-4 lg:px-6 sm:pt-6">
      <div class="overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-sm ring-1 ring-slate-900/5">
        <template v-if="archives.data.length">
          <ul class="divide-y divide-slate-100">
            <li v-for="a in archives.data" :key="a.id">
              <Link
                :href="route('tour-cabinet.archives.contest.show', a.id)"
              ...
              >
                <div class="flex flex-wrap items-start justify-between gap-3">
                  <div class="min-w-0 flex-1">
                    ...
                    <p v-if="a.direction_label" class="mt-1 text-sm text-slate-700">{{ a.direction_label }}</p>
        ...
        <p v-else class="px-6 py-12 text-center text-sm text-slate-600">Архивных заявок на конкурс пока нет.</p>
      </div>
    </div>
  </div>
</template>
