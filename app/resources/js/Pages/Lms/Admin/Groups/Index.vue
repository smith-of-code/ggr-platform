<template>
  <LmsAdminLayout :event="event">
    <div class="mb-8 flex items-center justify-between">
      ...
      <Link :href="route('lms.admin.groups.create', event.id)" class="flex items-center gap-2 rounded-xl bg-rosatom-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-rosatom-700">
          ...
          <tr v-for="g in groups.data" :key="g.id" class="transition hover:bg-gray-50">
            ...
            <td class="px-5 py-3.5 text-right">
              <div class="flex items-center justify-end gap-2">
                <Link :href="route('lms.admin.groups.edit', [event.id, g.id])" class="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 hover:text-gray-900">
                ...
                <button @click="confirmDestroy(g)" class="rounded-lg p-2 text-gray-500 transition hover:bg-red-50 hover:text-red-600">
...
function confirmDestroy(group) {
  if (confirm(`Удалить группу "${group.title}"?`)) {
    router.delete(route('lms.admin.groups.destroy', [props.event.id, group.id]))
  }
}
</script>
