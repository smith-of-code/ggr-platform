<template>
  <LmsAdminLayout :event="event">
    <div class="mb-8 flex items-center justify-between">
      ...
      <Link :href="route('lms.admin.trajectories.create', event.id)" class="flex items-center gap-2 rounded-xl bg-rosatom-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-rosatom-700">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="t in trajectories.data" :key="t.id" class="transition hover:bg-gray-50">
            ...
            <td class="px-5 py-3.5 text-center">
              <span :class="t.is_active ? 'bg-rosatom-50 text-rosatom-700' : 'bg-gray-100 text-gray-500'" class="rounded-full px-3 py-1 text-xs font-semibold">
                {{ t.is_active ? 'Активна' : 'Скрыта' }}
              </span>
            </td>
            <td class="px-5 py-3.5 text-right">
              <div class="flex items-center justify-end gap-2">
                <Link :href="route('lms.admin.trajectories.edit', [event.id, t.id])" class="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 hover:text-gray-900">
                ...
                <button @click="confirmDestroy(t)" class="rounded-lg p-2 text-gray-500 transition hover:bg-red-50 hover:text-red-600">
...
function confirmDestroy(trajectory) {
  if (confirm(`Удалить траекторию "${trajectory.title}"?`)) {
    router.delete(route('lms.admin.trajectories.destroy', [props.event.id, trajectory.id]))
  }
}
</script>
