<template>
  <LmsAdminLayout :event="event">
    <div class="mb-8 flex items-center justify-between">
      ...
      <Link :href="route('lms.admin.courses.create', event.id)" class="flex items-center gap-2 rounded-xl bg-rosatom-600 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-rosatom-700">
      ...
      <table class="min-w-full">
        <thead>
          <tr class="border-b border-gray-200 bg-gray-50">
            ...
            <th class="px-5 py-3 text-center text-xs font-semibold uppercase tracking-wider text-gray-500">Записей</th>
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="course in courses.data" :key="course.id" class="transition hover:bg-gray-50">
            <td class="px-5 py-3.5">
              <div class="flex items-center gap-3">
                <div v-if="course.image" class="h-10 w-14 shrink-0 overflow-hidden rounded-lg bg-gray-100">
            ...
            <td class="px-5 py-3.5 text-center text-sm text-gray-500">{{ course.stages?.length ?? 0 }}</td>
            <td class="px-5 py-3.5 text-center text-sm text-gray-500">{{ course.enrollments_count ?? 0 }}</td>
            <td class="px-5 py-3.5 text-center">
              <span :class="course.is_active ? 'bg-rosatom-50 text-rosatom-700' : 'bg-gray-100 text-gray-500'" class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold">
                <span :class="course.is_active ? 'bg-rosatom-600' : 'bg-gray-400'" class="h-1.5 w-1.5 rounded-full" />
                {{ course.is_active ? 'Активен' : 'Скрыт' }}
              </span>
            </td>
            <td class="px-5 py-3.5 text-right">
              <div class="flex items-center justify-end gap-2">
                <Link :href="route('lms.admin.courses.edit', [event.id, course.id])" class="rounded-lg p-2 text-gray-500 transition hover:bg-gray-100 hover:text-gray-900">
                ...
                <button @click="confirmDestroy(course)" class="rounded-lg p-2 text-gray-500 transition hover:bg-red-50 hover:text-red-600">
...
function confirmDestroy(course) {
  if (confirm(`Удалить курс "${course.title}"?`)) {
    router.delete(route('lms.admin.courses.destroy', [props.event.id, course.id]))
  }
}
</script>
