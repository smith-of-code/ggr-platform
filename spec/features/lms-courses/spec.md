...
| scheduled_at | timestamp | nullable, дата живого события |
| scheduled_ends_at | timestamp | nullable, дата окончания живого события |
...
1. **Админка (`Form.vue`, `StageEditor.vue`)**: этапы модуля и этапы без модуля выводятся списком; **после каждого этапа** есть кнопка «Добавить этап» (вставка нового этапа сразу ниже, без прокрутки к концу страницы). В `StageEditor` у этапа есть поле `available_from` («Открытие этапа») и список блоков; кнопка «Добавить блок» создаёт новый блок.
...
3. **Отображение (`Show.vue`, `Stage.vue`)**: на странице программы участник видит дату и время открытия этапа, если задано `available_from`, для этапов внутри модуля и для этапов без модуля. Время выводится из сохранённой строки без браузерного timezone-сдвига, чтобы `10:10` из админки отображалось как `10:10`. Бэкенд передаёт массив `blocks` с загруженными связями на страницу этапа; Vue рендерит каждый блок в соответствии с его типом.
4. **Защита отметки прохождения для датированных событий**: для блоков `workshop`, `city_meeting`, `curator_meeting` с заполненным `scheduled_at` участник не может отметить этап пройденным до наступления этой даты. На фронте кнопка `Отметить как пройденное` отключается и показывает сообщение `Отметить можно только после наступления даты события`; backend в `StageController@complete` повторно проверяет дату и возвращает blocking error.
...
Модули имеют даты доступности (`available_from`, `available_to`), тип разблокировки (`unlock_type`). Этапы привязаны к модулю через `lms_course_module_id` (nullable — этап без модуля). Каждый этап, включая этапы внутри модуля и этапы без модуля, может иметь собственную дату открытия `available_from`; участник не может открыть этап до этой даты.
...
7. **Sequential в модулях**: при `sequential=true` последовательность рассчитывается внутри своей цепочки (этапы без модуля — отдельная цепочка, каждый модуль — отдельная цепочка). Этапы вне модуля не блокируют открытие этапов внутри модулей.
...
- В Excel-выгрузку участников (`users-export`) добавлена колонка «Описание идеи / проекта» из `lms_profiles.project_description`.
