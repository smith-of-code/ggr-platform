...
Развёрнутые строки занятий используют ту же базу `enrollment` программы:
- `Записано` у занятия = количество участников, записанных на программу занятия.
- `Открыли этап` = участники, у которых есть запись в `lms_stage_progress` по этому занятию.
- `Не начали` = записанные на программу минус участники со статусами `in_progress` / `completed` по занятию.
- `В процессе` / `Завершено` = статусы `lms_stage_progress` по конкретному занятию.

Поэтому корректен сценарий, когда на программу записано 144 участника, но конкретное занятие ещё никто не начал: у занятия будет `Записано = 144`, `Открыли этап = 0`, `Не начали = 144`.

Порядок занятий в развёрнутом отчёте строится автоматически по текущей структуре программы: позиция программы → этапы без модуля → позиция модуля → позиция этапа. При изменении порядка в программе отдельная синхронизация не нужна: отчёт каждый раз читает актуальные `position` из `lms_courses`, `lms_course_modules`, `lms_course_stages`.
