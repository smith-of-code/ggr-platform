...
1. **UI_TWEAK**: Блоки «Направления» и «Для кого» визуально сливаются — в «Для кого» заменить нумерацию (1, 2, 3) на иконки, сделать карточки визуально отличными от секции «Направления».
2. **UI_TWEAK**: Убрать звёздочку (`target_audience_note`) внизу блока «Для кого» — текст об иностранных гражданах уже содержится в описаниях аудиторий.
3. **UI_TWEAK**: Визуально показать, что «Развёрнутые ответы» и «Проверочное задание» относятся к конкурсу — переместить под/внутрь блока «Победить в конкурсе», расположить вертикально (Проверочное задание под Развёрнутыми ответами).
4. **BEHAVIOR_CHANGE**: Кнопки «Личный кабинет» и «Оставить заявку» — ошибка при клике. Исправить: «Оставить заявку» не должна использовать Inertia Link с POST-маршрутом, заменить на scroll к турам.
