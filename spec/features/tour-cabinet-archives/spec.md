...
- ~~Экспорт архивных заявок в CSV/Excel — пока работает существующий экспорт `admin.tour-cabinet.tour-users.export` (он берёт текущий прогресс + applications); расширение экспорта архивами — потенциальная следующая фича.~~ **Реализовано в фиче `admin-tour-users-commerce-archives`** (см. `spec/features/admin-tour-users-commerce-archives/spec.md`): экспорт `admin.tour-cabinet.tour-users.export` теперь дополнительно учитывает архивы коммерческих туров и фильтр по городу также видит `tour_cabinet_commerce_archives.city_id`. Архивы конкурса в CSV в эту фичу не вошли — `payload` конкурса крупный и многоуровневый, отдельная фича при необходимости.
