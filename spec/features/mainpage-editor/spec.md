...
Дать администратору возможность редактировать все контентные блоки страницы `/mainpage` и управлять их порядком через раздел админки, по образцу `admin/opportunity-tours-page` и `admin/vshgr-page`.
...
- Редактор страницы `Home.vue` (корень `/`) — только `/mainpage`.
