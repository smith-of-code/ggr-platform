...
8. **Синхронизация Направлений и Проектов**: заказчик считает «Направления» и «Проекты» одной и той же сущностью. Сейчас «Проекты» — захардкоженная константа `Tour::PROJECTS` (3 ключа), а «Направления» — динамическая модель `Direction` с CRUD. Между ними нет автоматической синхронизации. Требуется: при создании/активации Direction автоматически появляется project в каталоге и ЛК туров; при скрытии/удалении Direction — исчезает project. Затрагивает 15+ вхождений `Tour::PROJECTS` в 9+ PHP-файлах + хардкод в `Tours/Index.vue`. Решение: заменить `Tour::PROJECTS` на динамический источник из `Direction` модели (например, `Direction::projectMap()`).
