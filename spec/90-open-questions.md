...
11. ~~**Коллизия URL `/directions/atomy-vkusa`: Direction vs AtomsVkusa Грантовый конкурс**~~: решено в фиче `atomy-vkusa-url-split` (вариант «а»). Грантовый конкурс выехал на отдельный URL `/atomy-vkusa` (`AtomsVkusaPageController@show`), а `/directions/atomy-vkusa` теперь работает как обычная страница `Direction` (`Directions/Show.vue`). Override-ветка в `DirectionController::show()` удалена, шапка сайта и `config/page_visibility.php` обновлены.
